...
function ProductCard(p) {
  ...
  return h("article", { class: "pcard fade-in", "aria-label": localizedProductName(p), onclick: () => Router.go("/product/" + p.id) },
...
function ProductDetailPage(id) {
  ...
  const wrap = h("div", { class: "product-detail-wrap", style: { maxWidth: "1560px", margin: "0 auto", padding: "30px 22px" } });
  const render = () => {
    const image = productImage(p);
    const thumbs = [image, image, image].map((src, i) => h("div", { class: "card product-detail-thumb", style: { backgroundImage: `url('${src}')`, borderColor: i === 0 ? "var(--pri)" : "var(--line)" } }));
    const media = h("div", {}, h("div", { class: "card product-detail-media", style: { backgroundImage: `linear-gradient(180deg,rgba(31,43,30,.02),rgba(31,43,30,.18)),url('${image}')` } }, h("span", { class: "badge-sale" }, "SAVE " + p.discount + "%")), h("div", { style: { display: "flex", gap: "10px", marginTop: "12px" } }, thumbs));
...
function CheckoutPage() {
  ...
  const render = () => {
    ...
    const cryptoBox = data.paymentMethod === "crypto" ? h("div", { class: "card", style: { display: "grid", gap: "10px", padding: "14px", borderColor: "var(--acc)" } }, h("strong", {}, t("cryptoPayment")), h("p", { style: { color: "var(--mut)", fontSize: "12px", margin: 0 } }, t("cryptoNotice")), h("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" } }, h("label", {}, t("cryptoAsset"), assetSelect), h("label", {}, t("network"), networkSelect))) : null;
     ...
     const summaryRows = d.items.map((i) => h("div", { style: { display: "flex", gap: "10px", padding: "10px 0", borderBottom: "1px solid var(--line)", alignItems: "center" } }, h("span", { class: "media-thumb media-thumb-sm", style: { backgroundImage: "url(" + productImage(i.product) + ")" } }), h("span", { style: { flex: 1, fontSize: "12px" } }, i.product.name.slice(0, 28), " ×", i.qty), h("strong", {}, money(i.line))));
...
function OrderPage(id) {
  ...
  const items = o.items.map((i) => h("div", { style: { display: "flex", alignItems: "center", gap: "10px", padding: "10px 0", borderBottom: "1px solid var(--line)" } }, h("span", { class: "media-thumb media-thumb-sm", style: { backgroundImage: "url(" + productImage(Store.product(i.productId) || { category: "electronics" }) + ")" } }), h("span", { style: { flex: 1, fontSize: "13px" } }, i.name, " ×", i.qty), h("strong", {}, money(i.price * i.qty))));
...
function AfrinProductCard(p, ref) {
  const saved = Store.inWishlist(p.id);
  return h("article", { class: "pcard fade-in", "aria-label": t(ref.name), onclick: () => Router.go("/product/" + p.id) },
